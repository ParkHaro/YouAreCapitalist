...
{
    public float income;           // 직접 저장 (빠른 접근)
    public float wealth;
    public PopulationClass class;  // 열거형으로 정규화
}
...
#### 데이터 지역성 최적화
- **Hot Data**: 매 프레임 접근하는 데이터 → 별도 컴포넌트
- **Cold Data**: 가끔 접근하는 데이터 → SharedComponent 또는 별도 저장
- **Static Data**: 변하지 않는 데이터 → ScriptableObject
...
{
    public float optimismLevel;      // 낙관/비관 성향
    public float conformityLevel;    // 동조 성향 (군중심리)
    public float loyaltyLevel;       // 브랜드/회사 충성도
    public float informationLevel;   // 정보 수준 (합리적 의사결정)
}
```
...
{
    LowIncome = 0,      // 저소득층: 월소득 200만원 이하
    LowerMiddle = 1,    // 중하위층: 200-350만원
